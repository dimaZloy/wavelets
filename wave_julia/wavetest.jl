...
include("sst_nino3.jl")   # input SST time series
include("wavelet.jl") # load prime functions 
...
m = mean(sst);
variance = stdm(sst,m)^2;
sst = (sst .- m)./sqrt(variance) ;

n = length(sst);
dt = 0.25 ;

#time = [0:length(sst)-1]*dt + 1871.0 ;  # construct time array
time = LinRange(0, length(sst)-1,n);
time = time.*dt .+ 1871.0; 
...
contourf(tc,log2.(yc),log2.(power),log2.(levels));  #*** or use 'contourfill'
